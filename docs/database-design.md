...
Database name: `govtech_db`

```sql
CREATE DATABASE IF NOT EXISTS govtech_db;
```
Tables are auto-generated by Hibernate (`spring.jpa.hibernate.ddl-auto=update`) based on the JPA entities — no manual schema scripts are required.